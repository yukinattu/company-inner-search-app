...
from dotenv import load_dotenv  # ← ローカル対応のため追加

# ✅ .env または secrets から APIキーを読み込み、環境変数にセット
load_dotenv()  # .env読み込み（ローカル用）
...
if chat_message:
    ...
    with st.spinner(ct.SPINNER_TEXT):
        try:
            ...
            # エラーメッセージの画面表示
            st.error(utils.build_error_message(ct.GET_LLM_RESPONSE_ERROR_MESSAGE))
            # 後続の処理を中断
            st.stop()
